...
COPY --chown=www-data:www-data docker/php/php.ini /usr/local/etc/php/conf.d/zz-warcry.ini
COPY --chown=www-data:www-data docker/apache/000-default.conf /etc/apache2/sites-available/000-default.conf
COPY --chown=www-data:www-data docker/entrypoint.sh /usr/local/bin/warcry-entrypoint.sh
