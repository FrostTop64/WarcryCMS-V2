		...
		function(data)
		{
			...
			else
			{
				...
				else
				{
					...
					var safePoint = String($SelectedPoint).replace(/[^0-9]/g, '');
						$('<input>', { type: 'hidden', name: 'point', value: safePoint }).appendTo($this);
					//submit the form
					$this.submit();
				}
			}
		...
		WarcryQueue('TELEPORTER').add(function()
		{
			$.get(
				...
				function(data)
				{
					...
					{
						$levelHolder.text(minLevel + ' Level');
					}
					else
					{
						$levelHolder.text(minLevel + ' Level - ' + maxLevel + ' Level');
					}
