	...
	var methods =
	{
		...
		init : function(options)
		{
			...
			$('#armors-filter-select').on('change', function()
			{
				...
				{
					var query_arg_regex;
				    query_arg_regex = /([?&])filter=[^&]*/i;
				    currentQuery = currentQuery.replace(query_arg_regex, '$1filter=' + encodeURIComponent($filter));
				}
				else
				{
					currentQuery = currentQuery + '&filter=' + encodeURIComponent($filter);
				}
