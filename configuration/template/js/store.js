	...
	var methods =
	{
		...
		updatePagination : function(options)
		{
			...
			WarcryQueue('STORE_NAVS').add(function()
			{
				...
				info.empty();
					if (leftSep) { $('<p>').html('|&nbsp;&nbsp;').appendTo(info); }
					info.append(document.createTextNode(offset + '-' + (parseInt(offset, 10) + parseInt(count, 10)) + ' of ' + $config.totalRecords));
					if (rightSep) { $('<p>').html('&nbsp;&nbsp;|').appendTo(info); }
