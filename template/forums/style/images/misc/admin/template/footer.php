...
$(function(){
  ...
  $('#search').on('keyup', function(){ var q=$(this).val().toLowerCase(); $('#content table tbody tr, #content .admin-card, #content .box, #content .message, #content .quick-card, #content .stat-card').each(function(){ $(this).toggle($(this).text().toLowerCase().indexOf(q)>-1); }); });
});
</script>
